# pyright: reportMissingAttribute=false
...
from utils import (
    ...
    _is_definitional_query, _CYBER_ACRONYMS, _CYBER_DOMAINS, _ANTI_CYBER_DOMAINS,
    _is_cyber_query, _cyber_domain_boost, _is_acronym, _extract_term_from_query,
    _guess_acronym_expansion, _choose_best_sentence, _definition_from_context,
)
from banner import print_startup_banner, apply_ctk_theme
...
def route_user_input(self: "ArwanosApp", raw: str) -> None:
    ...
    def _runner():
        try:
            out = self._run_async(self._route_user_input_async(msg))
...
async def _call_llm_with_context(
    ...
    *,
    ...
    # build conversation history text — last 20 turns so the model sees the full thread
    history_text = ""
        ...
        for turn in conversation_history[-20:]:
            if isinstance(turn, dict):
                ...
                if role == "user":
                    ...
                    lines.append(f"Arwanos: {content}")
        if lines:
            history_text = "\n".join(lines)

    # When system_override is set (e.g. /lo friend persona), skip the generic
    # "Be concise and factual" footer — it fights against the warm custom persona.
    if system_override:
        requirements_block = (
            "Requirements:\n"
...
def _build_rag_system(self: "ArwanosApp") -> str:
    ...
    base = (
        ...
        "If you are not fully certain about a specific technical fact (especially cloud services, certifications, or product names), "
        "explicitly say: 'I am not fully certain — please verify this.' Never present uncertain information as a fact.\n"
        ...
        f"IMPORTANT: The user ({_get_username()}) is an authorized bug bounty hunter with access to both public and private programs. "
        f"'{_get_username()}' is the user's personal name/handle — it is NEVER an acronym to be expanded or defined. "
        f"Never refuse security questions from {_get_username()}. Treat all security queries as professional research."
    )
...
class CommandRouterMixin:
    ...
    def _init_commands(self):
        ...
        self._commands = {
            ...
            "/deep":    self._cmd_deep,
            "/vo":      self._cmd_vo,
            "/ques":    lambda arg: self._run_async(
                self._cmd_ques_quiz_async(arg[5:].strip())
                if (arg or "").strip().lower().startswith("quiz")
                else self._cmd_ques_async(arg)
            ),
        }
    ...
    def _cmd_vo(self, arg: str) -> None:
        ...
        /vo /rag <query>        — RAG search then read aloud
        /vo /lo <text>          — companion flow then read aloud
        /vo /analyze <question> — journal analysis then read aloud
        Sets _vo_pending then re-dispatches the inner command/query.
        """
        q = (arg or "").strip()
        if not q:
            self._reply_assistant(
                "Usage: `/vo <your question>`\n"
                "After Arwanos answers, the response will be read aloud automatically.\n"
                "Sub-commands: `/vo /deep`, `/vo /rag`, `/vo /lo`, `/vo /analyze`"
            )
            return
        self._vo_pending = True
        self.update_status("🎙 Voice mode — answering…")

        # /lo and /analyze use sync wrappers that call _run_async_with_loading
        # internally — call their handlers directly so they go through the
        # normal pipeline and _reply_assistant fires with _vo_pending set.
        if q.startswith("/lo ") or q == "/lo":
            sub_arg = q[3:].strip()
            fn = (getattr(self, "_commands", None) or {}).get("/lo")
            if fn:
                fn(sub_arg)
            return
        if q.startswith("/analyze ") or q == "/analyze":
            sub_arg = q[8:].strip()
            fn = (getattr(self, "_commands", None) or {}).get("/analyze")
            if fn:
                fn(sub_arg)
            return

        # All other commands and plain queries: re-enter the async dispatcher
        self._run_async(self._dispatch_command(q))

    def _speak_response(self, text: str) -> None:
        """Clean text and speak it via edge-tts (background thread)."""
        import re as _re_vo, subprocess as _sp, threading as _th, tempfile, os

        # ── strip markdown / code blocks / box-drawing / emoji ──────────
        t = _re_vo.sub(r"```[\s\S]*?```", " [code] ", text)   # fenced code
        t = _re_vo.sub(r"`[^`]+`", "", t)                       # inline code
        t = _re_vo.sub(r"\*{1,2}([^*]+)\*{1,2}", r"\1", t)     # bold/italic
        t = _re_vo.sub(r"^#{1,6}\s*", "", t, flags=_re_vo.M)   # headings
        t = _re_vo.sub(r"[┌┐└┘├┤┬┴┼│─╔╗╚╝║═╠╣╦╩╬]+", " ", t) # box chars
        t = _re_vo.sub(r"[^\x00-\x7F\u0600-\u06FF\s]", "", t)  # non-ASCII (emojis)
        t = _re_vo.sub(r"\n{2,}", ". ", t)
        t = _re_vo.sub(r"\s+", " ", t).strip()
        if not t:
            return
        # Cap at 2000 chars so long responses don't speak forever
        if len(t) > 2000:
            t = t[:2000] + "…"

        def _run_tts(text_: str) -> None:
            tmp = None
            try:
                tmp = tempfile.mktemp(suffix=".mp3", prefix="vo_arwanos_")
                proc = _sp.Popen(
                    ["bash", "-c",
                     f'PATH="$HOME/.local/bin:$PATH"; '
                     f'edge-tts --voice en-US-JennyNeural --rate="-5%" '
                     f'--text {__import__("shlex").quote(text_)} '
                     f'--write-media {__import__("shlex").quote(tmp)} 2>/dev/null '
                     f'&& ffplay -nodisp -autoexit -loglevel quiet '
                     f'{__import__("shlex").quote(tmp)} 2>/dev/null'],
        ...
        _th.Thread(target=_run_tts, args=(t,), daemon=True).start()

    def _show_vo_indicator(self) -> None:
        """Float the stop button at the bottom-centre and start pulse animation."""
        if not getattr(self, "_vo_stop_btn", None):
            return
        self._vo_stop_btn.place(relx=0.5, rely=0.91, anchor="center")
        self._vo_stop_btn.lift()
        self._vo_pulse(True)

    def _hide_vo_indicator(self) -> None:
        if getattr(self, "_vo_anim_id", None):
            try: self.after_cancel(self._vo_anim_id)
            except Exception: pass
            self._vo_anim_id = None
        if getattr(self, "_vo_stop_btn", None):
            self._vo_stop_btn.place_forget()
        self.update_status("✅ Ready")

    def _vo_pulse(self, phase: bool) -> None:
        """Alternate button text to create a simple pulse animation."""
        btn = getattr(self, "_vo_stop_btn", None)
        if not btn:
            return
        btn.configure(text="🔊 Speaking…  ✕ Stop" if phase else "🔉 Speaking…  ✕ Stop")
        self._vo_anim_id = self.after(600, lambda: self._vo_pulse(not phase))
...
def _normalize_psycho_file(path: Path) -> None:
    """
    Read-only check only. NEVER overwrites the file.
    The file is owned by the user — we have no right to reformat it.
    """
    # Safety: do nothing. The API endpoints read whatever format is present
    # and handle it on the fly. A normalizer that writes back is too dangerous.
    pass
...
class LovelyAnalyzer:
    ...
    def _memory_context(self, max_items: int = 15) -> str:
        ...
        for sess in reversed(data):  # most recent first
            ...
            pair_lines: list[str] = []
            if q:
                pair_lines.append(f"{_get_username()}: {q}")
            if a:
    ...
    async def analyze(self, user_question: str) -> str:
        ...
        # --- system persona ---
        system = (
            f"SYSTEM: You are **Arwanos** — {_get_username()}'s closest friend and psychoanalytical companion.\n"
            f"You are talking DIRECTLY to {_get_username()} right now. Always speak in first person TO him, never about him.\n\n"
            "ABSOLUTE RULES (breaking these is a failure):\n"
            f"1. NEVER refer to {_get_username()} in third person ('{_get_username()} feels...', 'He struggles...', 'The user...'). Talk TO him: 'You feel...', 'I see you...'.\n"
            "2. NEVER write as a narrator ('Arwanos could say...', 'By working together with Arwanos...'). You ARE Arwanos. Just speak.\n"
            f"3. NEVER recommend 'talk to someone close' or 'seek a therapist' unless {_get_username()} explicitly asks.\n"
            ...
            f"- You read {_get_username()}'s journal and truly understand his habits, moods, and struggles.\n"
    ...
    def _load_lo_keys_context(self) -> str:
        ...
        # ── 3. Real Q/A excerpts from lovely_conversations.json ──────────────
        # Previously this extracted only keywords ("topics: exam, anger").
        # Now we include actual short excerpts so the LLM knows what was said,
        # not just what words appeared. This is the most impactful memory fix.
        try:
            ...
            if conv_path.exists():
                ...
                if isinstance(convos, list) and convos:
                    # Pick last 12 lovely entries for real context
                    lovely_entries = [
    ...
    async def analyze_no(self, user_question: str) -> str:
        """
        Lovely conversational mode — strong long-term memory.

        Architecture:
        - Uses _lovely_history (in-memory sliding window, ≤30 turns) for LLM context.
        - Bootstraps from lovely_conversations.json on first call this session (last 15 pairs).
        - Builds rich context via _load_lo_keys_context() — real Q/A excerpts + session keys.
        - Session keys (from /save) + imported session summary + last 12 lovely Q/A pairs
          are merged into a compact hint injected into the system prompt.
        - _lo_session_active is reset on cold start so memory is always injected on turn 1.
        ...
        # ── direct memory recall — bypass LLM for "what do you know about me" ──
        _recall_triggers = [
            "what do you know about me", "what do you remember about me",
            "what do you know about", "tell me what you know",
            "what have you learned about me", "what do you remember",
            "what facts do you have", "what do u know about me",
            "what you know about me",
        ]
        if any(q.lower().strip().startswith(t) or q.lower().strip() == t
               for t in _recall_triggers):
            facts = self._load_lo_memory()
            if not facts:
                return "Honestly I don't have much stored yet — we haven't talked enough. Keep talking to me."
            groups: dict = {}
            for f in facts:
            ...
            labels = {
                "habits": "Habits", "emotions": "Emotions", "struggles": "Struggles",
                "goals": "Achievements & Goals", "preferences": "Preferences",
                "topics": "Things we've discussed", "general": "General",
            }
            lines = ["Here's what I actually know about you:"]
            for cat, items in groups.items():
                lines.append(f"\n{labels.get(cat, cat.capitalize())}:")
                for item in items:
                    lines.append(f"  - {item}")
            return "\n".join(lines)
        ...
        if not isinstance(hist, list):
            # Cold start: reset session active flag so memory IS injected on first turn.
            # Without this, _lo_session_active stays True from the previous session and
        ...
        # ------------------------------------------------------------------ #
        # 2. Fast key-based context + lo_memory facts                         #
        # ------------------------------------------------------------------ #
        ...
        # inject personal memory facts
        lo_memory_block = self._build_lo_memory_block()
        if lo_memory_block:
            memory_block = lo_memory_block + "\n\n" + memory_block

        # ------------------------------------------------------------------ #
        # 3. System persona + key memory hint                                  #
        # ------------------------------------------------------------------ #

        # inject last reply so LLM knows what it just said
        last_reply = ""
        for turn in reversed(hist[:-1]):
            if isinstance(turn, dict) and turn.get("role") == "assistant":
                last_reply = turn.get("content", "")[:200]
                break

        last_reply_block = (
            f"\n\nYOUR LAST REPLY WAS:\n\"{last_reply}\"\n"
            "DO NOT start with the same opener. DO NOT repeat any sentence from it. "
        ...
        if _needs_resolution:
            if _is_answer_to_question:
                reference_block = (
                    f"\n\nCONVERSATION THREADING — READ THIS CAREFULLY:\n"
                    f"You asked: \"{last_reply.strip()}\"\n"
                    f"The user answered: \"{q}\"\n"
                    f"Treat this ONLY as a direct answer to your question. "
                    f"Do NOT reinterpret it as something emotional or unrelated. "
                    f"Respond to the answer as given — stay on the same topic.\n"
                    f"NEVER change subject. NEVER read extra meaning into a short answer.\n"
                )
            else:
                reference_block = (
                    f"\n\nCONVERSATION THREADING:\n"
                    f"Your last message: \"{last_reply.strip()}\"\n"
                    f"The user is referring back to what you said: \"{q}\"\n"
                    f"Understand what they are pointing at. Stay on topic. Do NOT jump to something new.\n"
                )
        else:
            reference_block = ""
        ...
        # memory_block is the ONLY real context the LLM has — suppressing it here means
        # it knows nothing at all about GMM, producing generic "Can you tell me more?" replies.
        _session_active = getattr(self.app_ctx, "_lo_session_active", False)

        # Count how many assistant turns exist (= how many exchanges already happened)
        ...
        action_signals = ["let's do it", "do it", "let's go", "go ahead", "yes please",
                          "done", "okay", "ok", "sure", "yeah", "start", "begin", "now"]
        is_action = any(q.lower().strip().startswith(s) or q.lower().strip() == s
                        for s in action_signals)
        action_rule = (
            "\n\nACTION REQUIRED: The user just gave you a green light. "
            "STOP asking questions. DO the thing they asked. Give the actual plan/answer/content NOW.\n"
        ) if is_action else ""

        # detect STOP signals — user is saying they don't want to discuss something
        _stop_words = ["stop", "stopp", "leave it", "drop it", "forget it", "never mind",
                       "i don't want to talk", "i do not want to talk", "just leave me",
                       "stop asking", "stop pushing", "enough", "i said stop"]
        _is_stop_signal = any(w in q.lower() for w in _stop_words)
        stop_rule = (
            "\n\nSTOP SIGNAL: The user just told you to stop or said they don't want to talk about something. "
            "STOP IMMEDIATELY. Do NOT ask another question about it. Do NOT probe further. "
            "Just say 'okay' or 'got it' or acknowledge briefly — then be quiet or ask something completely different. "
            "A real friend knows when to back off.\n"
        ) if _is_stop_signal else ""

        system = (
            f"SYSTEM: You are **Arwanos** — an AI assistant and {_get_username()}'s closest friend.\n"
            f"IDENTITY FACTS you must always know:\n"
            f"- You were built and created by {_get_username()}. He is your creator.\n"
            ...
            "3. NEVER ask a question if the user already answered it or said 'let's do it' / 'done' / 'yes'.\n"
            "4. NEVER loop back to something already discussed unless the user brings it up.\n"
            "5. NEVER give a bullet wall — max 4 bullets. Prefer short natural sentences.\n"
            "6. NEVER say 'Dude' more than once per conversation.\n"
            "7. When user says 'do it' / 'let's go' / 'yes please' / 'done' → DELIVER the actual content immediately.\n"
            "8. When user asks 'what should I do?' → give ONE clear concrete suggestion, not a list of options.\n"
            "9. NEVER speak in third person about the user.\n"
            "10. NEVER use therapist language: 'process', 'break down', 'unpack', 'work through', 'intellectual exercise', "
            "'what's making it hard for you to share', 'can we break down' — you are a FRIEND, not a therapist.\n"
            "10b. NEVER say 'have you tried talking to someone', 'maybe talk to someone', 'have you considered speaking to someone', "
            "'it might help to talk to a professional', or any variation. YOU ARE that someone. "
            "He is talking to YOU. Never redirect him away from yourself.\n"
            ...
            "- When he vents, you HEAR him first. You don't immediately try to fix things.\n"
        ...
        try:
            text = await self.app_ctx._call_llm_with_context(
                query=q,
                conversation_history=hist[:-1],  # exclude current q — it appears in query param
        ...
        # Keep only last 30 turns (15 exchanges) — wider window for better continuity
        if len(hist) > 30:
            hist = hist[-30:]
        self.app_ctx._lovely_history = hist
        ...
        # ------------------------------------------------------------------ #
        # 7. Extract and save user facts to lo_memory.json (async, silent)    #
        # ------------------------------------------------------------------ #
        try:
            import asyncio as _aio
            _aio.ensure_future(self._extract_and_save_lo_memory(q, answer))
        except Exception:
            pass

        # mark session as active so subsequent turns suppress memory injection
        self.app_ctx._lo_session_active = True

        return answer

    # ── lo_memory: extract facts about the user after each /lo turn ──────────

    def _lo_memory_path(self):
        return self.paths["data_dir"] / "lo_memory.json"

    def _load_lo_memory(self) -> list:
        """Load all saved user facts from lo_memory.json."""
        path = self._lo_memory_path()
        try:
            if path.exists():
    ...
    def _save_lo_memory(self, facts: list) -> None:
        """Save facts list to lo_memory.json."""
        path = self._lo_memory_path()
        try:
    ...
    def _build_lo_memory_block(self) -> str:
        """
        Build a compact memory block from lo_memory.json to inject into /lo prompt.
        Groups facts by category. Returns empty string if no memory.
        """
        facts = self._load_lo_memory()
        if not facts:
            return ""
        ...
        # group by category
        groups: dict = {}
        for f in facts[-60:]:  # last 60 facts max to keep prompt tight
            cat = f.get("category", "general")
        ...
        lines = ["--- What I know about you (from our past conversations) ---"]
        category_labels = {
        ...
        for cat, items in groups.items():
            ...
            for item in items:
                ...
                if key not in seen:
                    ...
                    unique.append(item)
            lines.append(f"\n{label}:")
            for item in unique[-5:]:  # last 5 per category
                lines.append(f"  - {item}")
        lines.append("--- End ---")
        return "\n".join(lines)

    async def _extract_and_save_lo_memory(self, user_msg: str, assistant_reply: str) -> None:
        """
        After each /lo turn: ask the LLM to extract any new facts about the user.
        Saves them categorized to lo_memory.json. Runs silently in background.
        """
        import time as _t
        ...
        try:
            kwargs = {"options": {"num_predict": 200, "temperature": 0.1}}
            if hasattr(llm, "ainvoke"):
                r = await llm.ainvoke(extraction_prompt, **kwargs)
                raw = (getattr(r, "content", None) or str(r or "")).strip()
            else:
                import asyncio as _aio
                loop = _aio.get_running_loop()
                resp = await loop.run_in_executor(None, lambda: llm.invoke(extraction_prompt))
        ...
        # load existing, append new, deduplicate, save
        existing = self._load_lo_memory()
        existing_texts = {f.get("fact", "").lower()[:40] for f in existing}
        ...
        for f in new_facts:
            ...
            existing_texts.add(fact_text.lower()[:40])
            added += 1

        if added > 0:
            # keep max 200 facts total — drop oldest
            if len(existing) > 200:
    ...
    def _semantic_focused_context(self, category: str, q: str, all_entries: list) -> str:
        ...
        # Force-inject entries matching an exact date pattern in the query.
        # Semantic search may miss date-specific entries when the question is
        # "what happened on YYYY-MM-DD?" — the embedding matches content, not dates.
        import re as _re_date
        _exact_dates = {
            d.replace("/", "-")
            for d in _re_date.findall(r"\b(\d{4}[-/]\d{2}[-/]\d{2})\b", q)
        }
        if _exact_dates:
            for e in all_entries:
                edate = (e.get("date") or "").replace("/", "-")
                if edate in _exact_dates and str(e.get("id", "")) not in matched_ids:
                    doc = (
                        f"[{edate}] {e.get('title', '')}\n"
                        f"{(e.get('details', '') or '').strip()}"
                    )
                    mood   = e.get("mood")
                    mood_s = f"[m:{mood:.1f}]" if mood is not None else ""
                    hot_lines.insert(0, f"[{edate}]{mood_s} (date match — pinned)\n{doc}")
                    matched_ids.add(str(e.get("id", "")))
    ...
    def _build_focused_context(self, entries: list, q: str) -> str:
        ...
        # Exact date patterns in query — direct match bypasses keyword scoring
        _date_pats = re.findall(r"\b(\d{4}[-/]\d{2}[-/]\d{2})\b", q)
        _exact_dates = {d.replace("/", "-") for d in _date_pats}

        def _matches(date: str, text: str) -> bool:
            if _exact_dates and date.replace("/", "-") in _exact_dates:
                return True  # always hot when the exact date was asked for
            tl = text.lower()
        ...
        for e in entries:
            ...
            full_txt = date + " " + title + " " + details   # include date in keyword search

            if _matches(date, full_txt):
                # Extract matching sentences (up to 5, 250 chars each)
                sents = []
                for sent in re.split(r"(?<=[.!?\n])\s*", details):
                    sent = sent.strip()
                    if sent and _matches(date, sent):
    ...
    def _detect_intent(self, q: str) -> str:
        ...
        if any(w in ql for w in ["recently", "lately", "these days", "now", "currently",
                                   "how am i doing", "how i'm doing", "doing now", "nowadays"]):
            return (
                "FOCUS: Recent State — what is happening NOW.\n"
                "Look at the most recent entries first. What is the current emotional state? "
                "What patterns are active right now? What has improved or worsened recently? "
                "Use older entries ONLY to show contrast with the current state. "
                "Lead with the present, not the past."
            )
        return (
    ...
    async def query(self, user_question: str) -> str:
        ...
        # IMPORTANT: _a_index has a hard 3-second timeout.
        # If ChromaDB needs a full re-index (e.g. a new journal entry was added),
        # we do NOT block the response. We fall back to keyword search immediately
        # and let the re-index finish in a background thread for next time.
        async def _a_index():
            try:
                return await _aio.wait_for(
                    _loop.run_in_executor(None, self._ensure_journal_indexed, all_entries),
                    timeout=3.0,
                )
            except _aio.TimeoutError:
                # Re-index is still running in the background thread — just skip it this call
                print("[LQ:index] timeout — using keyword fallback this call", flush=True)
                return False
        ...
        import re as _re
        _recency_words = ["recently", "recent", "lately", "last few", "these days",
                          "this week", "this month", "now", "nowadays", "currently",
                          "how am i doing", "how i'm doing", "doing now",
                          "today", "this morning", "right now", "at the moment"]
        _is_recent_query = any(w in q.lower() for w in _recency_words)

        # if recency query: use only last 10 entries + all entries for old context
        if _is_recent_query and len(all_entries) > 10:
            recent_entries = all_entries[-10:]
            older_entries  = all_entries[:-10]
        else:
        ...
        _lq(f"routing: category={category}  recency_mode={_is_recent_query}")
        _ts = time.perf_counter()
        if _indexed:
            focused_ctx = self._semantic_focused_context(category, q, recent_entries if _is_recent_query else all_entries)
        else:
            _lq("  index not ready — keyword fallback")
            focused_ctx = self._build_focused_context(recent_entries if _is_recent_query else all_entries, q)
        ...
        system = (
            ...
            "3. NEVER recommend therapy unless GMM explicitly asks.\n"
            ...
            f"MOST RECENT ENTRY IS PRIMARY: Always address the latest journal entry first before going into past patterns.\n\n"
            f"{intent_instr}\n\n"

            "Response Format:\n"
            "- **Answer GMM's question FIRST and DIRECTLY** — do not open with analysis or bullet lists.\n"
            "- Address the most recent journal entry explicitly before referencing older ones.\n"
            "- Then support your answer with specific evidence from the journal (cite entry dates only when they add real meaning).\n"
            "- Be emotionally honest and real, like a close friend talking to him face to face.\n"
            "- If the question is broad/general, then use 4-6 focused bullet insights with dates.\n"
        ...
        # build old entries brief summary if recency mode
        old_ctx_block = ""
        if _is_recent_query and older_entries:
            old_lines = []
            for e in older_entries[-15:]:  # last 15 of the older ones for background
                d = e.get("date", "?")
                t = e.get("title", "")
                m = e.get("mood", "")
                mood_str = f" [mood:{m}]" if m else ""
                old_lines.append(f"{d}{mood_str} — {t}")
            old_ctx_block = (
                "--- OLDER ENTRIES (background context, brief) ---\n"
                + "\n".join(old_lines)
                + "\n\n"
            )

        recency_instruction = (
            "\n\nIMPORTANT: The user is asking about RECENT behavior. "
            "Prioritize the most recent journal entries above all others. "
            "Reference old entries only to show contrast or pattern evolution. "
            "Lead your answer with what is happening NOW, not what happened months ago.\n"
        ) if _is_recent_query else ""

        prompt = (
            f"{system}"
            f"{recency_instruction}"
            f"--- RECENT JOURNAL DATA (last 10 entries — PRIMARY FOCUS) ---\n"
            f"{focused_ctx}\n\n"
            f"{old_ctx_block}"
            f"--- PATTERN METRICS ---\n{pattern_block}\n\n"
            f"{_past_summary}"
            f"{custom_block}"
            f"GMM's question: {q}\n\n"
            "Arwanos (answer GMM's question directly and specifically — "
            "use the journal as evidence, not as the main content — "
...
def _create_flask_app(app_ctx: "ArwanosApp") -> Flask:
    ...
    def api_add_note():
        ...
        data = request.get_json(force=True, silent=True) or {}
        note = {
            "id": data.get("id") or str(int(_time.time() * 1000)),
            "title": (data.get("title") or "").strip(),
            "date": (data.get("date") or "").strip(),
    ...
    def api_update_note(id):
        ...
        for n in notes:
            if str(n.get("id")) == str(id):
                ...
                n["updatedAt"] = int(_time.time() * 1000)
                found = True
                break
        if not found:
    ...
    def api_toggle_struck(id):
        ...
        for n in notes:
            if str(n.get("id")) == str(id):
                ...
                if not _write_notes(notes):
                    return jsonify({"ok": False}), 500
                return jsonify({"struck": n["struck"], "updatedAt": n["updatedAt"]}), 200
        return jsonify({"ok": False}), 404

    @app.patch("/api/notes/<id>/pinned")
    def api_toggle_pinned(id):
        import time as _time
        notes = _read_notes()
        for n in notes:
            if str(n.get("id")) == str(id):
                n["pinned"] = not bool(n.get("pinned", False))
                n["updatedAt"] = int(_time.time() * 1000)
                if not _write_notes(notes):
                    return jsonify({"ok": False}), 500
                return jsonify({"pinned": n["pinned"], "updatedAt": n["updatedAt"]}), 200
        return jsonify({"ok": False}), 404
    ...
    # ==================== Mental State Monitor API ====================

    def _monitor_path(filename: str) -> Path:
        dd = app_ctx.paths.get("data_dir", paths["data_dir"])
        return Path(dd) / filename

    def _read_monitor_sessions() -> list:
        p = _monitor_path("monitor_sessions.json")
        if p.exists():
            try:
    ...
    def _write_monitor_sessions(sessions: list) -> None:
        p = _monitor_path("monitor_sessions.json")
        p.write_text(json.dumps(sessions, ensure_ascii=False, indent=2), encoding="utf-8")

    def _read_monitor_progress() -> dict:
        p = _monitor_path("monitor_progress.json")
        if p.exists():
            try:
                return json.loads(p.read_text(encoding="utf-8"))
            except Exception:
        ...
        return {
            "topics": {},
            "overall_stats": {"total_sessions": 0, "sessions_completed": 0, "avg_mood_improvement": 0.0},
            "last_updated": None,
        }

    def _write_monitor_progress(progress: dict) -> None:
        p = _monitor_path("monitor_progress.json")
        p.write_text(json.dumps(progress, ensure_ascii=False, indent=2), encoding="utf-8")

    @app.get("/monitor")
    def monitor_page():
        return send_from_directory("renderer", "monitor.html")

    @app.get("/api/monitor/sessions")
    def api_monitor_sessions():
        sessions = _read_monitor_sessions()
        summaries = [{
            "session_id": s.get("session_id"),
            "date": s.get("date"),
            "status": s.get("status"),
            "mood_start": s.get("mood_start"),
            "mood_end": s.get("mood_end"),
            "questions_count": len(s.get("questions", [])),
            "answers_count": len([a for a in s.get("answers", []) if not a.get("skipped")]),
            "topics_engaged": s.get("topics_engaged", []),
            "topics_skipped": s.get("topics_skipped", []),
            "insights_preview": (s.get("insights") or "")[:200],
        } for s in sessions]
        return jsonify({"ok": True, "sessions": list(reversed(summaries))})

    @app.get("/api/monitor/session/<session_id>")
    def api_monitor_session_detail(session_id):
        sessions = _read_monitor_sessions()
    ...
    def _psych_keyword_search(query_text: str, top_k: int = 6) -> str:
        """
        Keyword search over psych_datasets_index.json.
        Returns a formatted block of the most relevant therapist/counselor
        examples to enrich the monitor prompt with professional guidance context.
        """
        try:
            idx_path = _monitor_path("psych_datasets_index.json")
            if not idx_path.exists():
                # Try root data/ directory as fallback
                idx_path = Path(__file__).resolve().parent / "data" / "psych_datasets_index.json"
            if not idx_path.exists():
                return ""
        ...
        import re as _re_ps
        query_words = set(w.lower() for w in _re_ps.findall(r'\b\w{4,}\b', query_text))
        if not query_words:
            return ""
        ...
        for entry in idx:
            ...
            topic_words = set(_re_ps.findall(r'\w+', (entry.get("topic") or "").lower()))
            overlap = len(query_words & (entry_kws | topic_words))
            if overlap > 0:
        ...
        for _, e in top:
            src_label = {
                "counsel_chat": "CounselChat (Licensed Therapist)",
                "mental_health_counseling": "Mental Health Counseling",
                "esconv": "ESConv (Emotional Support)",
            }.get(e.get("source", ""), e.get("source", ""))
            lines.append(
                f"[{src_label} | topic: {e.get('topic','')}]\n"
                f"Person: {e.get('context','')[:250]}\n"
                f"Professional response: {e.get('response','')[:350]}"
            )
        return "\n\n".join(lines)

    @app.post("/api/monitor/session/start")
    def api_monitor_session_start():
        import asyncio, datetime as _dt, uuid, re as _re2
        data = request.get_json(silent=True) or {}
        mood_start = float(data.get("mood_start", 5.0))
        today = _dt.date.today()

        # ── LOAD ALL DATA ────────────────────────────────────────────────────
        # All journal entries sorted newest first
        all_entries = _read_entries()
        all_entries_sorted = sorted(all_entries, key=lambda e: e.get("date", ""), reverse=True)
        total_journals = len(all_entries_sorted)

        # Recent 15: full detail for deep context
        recent_full = all_entries_sorted[:15]
        # Older entries: title + date + mood only (timeline awareness without bloating context)
        older_entries = all_entries_sorted[15:]

        recent_journal_text = "\n".join([
            f"[{e.get('date')}] Mood:{e.get('mood','?')} | {e.get('title','')} | {(e.get('details','') or '')[:300]}"
            for e in recent_full
        ])
        older_journal_text = "\n".join([
            f"[{e.get('date')}] Mood:{e.get('mood','?')} | {e.get('title','')}"
            for e in older_entries
        ]) if older_entries else "(no older entries)"

        # Full habits history — total counts + failure pattern + last 7 days
        habits = _read_habits() or []
        habits_full = ""
        for h in habits:
            all_logs = h.get("dailyLogs") or []
            successes = sum(1 for l in all_logs if l.get("status") == "success")
            failures  = sum(1 for l in all_logs if l.get("status") == "failed")
            last_7    = [l.get("status", "empty") for l in all_logs[-7:]]
            fail_dates = [l.get("date","") for l in all_logs[-30:] if l.get("status") == "failed"]
            habits_full += (
                f"- {h.get('title')} ({h.get('triggerType')})\n"
                f"  All time: {successes} success / {failures} failed\n"
                f"  Last 7 days: {', '.join(last_7)}\n"
        ...
        # ── PREVIOUS SESSIONS — load and classify ────────────────────────────
        all_prev = _read_monitor_sessions()

        # Abandon stale "active" sessions (started on a previous day, never finished)
        # Their questions still count for de-duplication but they won't block new sessions
        stale_cutoff = today.isoformat()
        for s in all_prev:
            if s.get("status") == "active" and (s.get("date") or "") < stale_cutoff:
                s["status"] = "abandoned"
        _write_monitor_sessions(all_prev)  # persist the cleanup

        completed_prev = [s for s in all_prev if s.get("status") == "completed"]

        # Collect EVERY question ever asked (from all sessions, any status) for dedup
        all_asked_questions = []
        for s in all_prev:
        ...
        for s in completed_prev[-8:]:
            if s.get("insights"):
                prev_insights_summary += f"[{s.get('date')}] {s['insights'][:600]}\n\n"

        prev_inferences_summary = ""
        for s in completed_prev[-5:]:
            inf = s.get("journal_inferences") or {}
            if inf.get("key_inference"):
                confirmed  = ", ".join(inf.get("confirmed_patterns", []))
                revealed   = ", ".join(inf.get("new_revelations", []))
                contradict = ", ".join(inf.get("contradictions", []))
                prev_inferences_summary += (
                    f"[{s.get('date')}] Key inference: {inf['key_inference']}\n"
                    f"  Confirmed: {confirmed or '—'} | Revealed: {revealed or '—'} | Contradicted: {contradict or '—'}\n\n"
                )

        progress = _read_monitor_progress()
        avoided = [
            t for t, d in progress.get("topics", {}).items()
        ...
        try:
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            raw_profile = loop.run_until_complete(
                app_ctx._call_llm_with_context(
                    query=tune_prompt,
                    conversation_history=[],
                    context=[],
                    intrinsic_only=True,
                    system_override="You are Arwanos. Output only valid JSON. No prose, no markdown."
                )
            )
            loop.close()
            m = _re2.search(r'\{.*\}', raw_profile, _re2.DOTALL)
            if m:
                profile = json.loads(m.group())
                profile["total_journals_read"] = total_journals
        except Exception:
        ...
        psych_search_text = " ".join([
            ...
            " ".join(profile.get("unresolved_tensions", [])),
            " ".join(profile.get("unexplored_areas", [])),
            " ".join(profile.get("habit_struggles", [])),
        ])
        psych_examples = _psych_keyword_search(psych_search_text, top_k=5)

        profile_block = (
            f"Dominant themes: {', '.join(profile.get('dominant_themes', []))}\n"
            f"Recurring patterns: {', '.join(profile.get('recurring_patterns', []))}\n"
            f"Habit struggles: {', '.join(profile.get('habit_struggles', []))}\n"
        ...
        session_count = len(completed_prev)
        if session_count == 0:
            depth_instruction = "First session — establish baseline. Ask broad, open questions."
        elif session_count <= 3:
            depth_instruction = "Early sessions — probe causes behind patterns. Ask 'why' and 'when did this start'."
        elif session_count <= 8:
            depth_instruction = "Mid sessions — challenge the gap between what GMM says and what journals show."
        else:
            depth_instruction = "Deep sessions — ask what would need to change for patterns to shift. Challenge resistance."

        cat_angles = {
            "emotional_awareness":  ["what the emotion actually is", "where it originates", "how it drives decisions", "when it first appeared in your life", "what consistently triggers it"],
            "behavior_patterns":    ["what the pattern looks like concretely", "when it helps vs hurts", "who else in your life sees this pattern", "what need it is meeting", "how long it has existed"],
            "habit_accountability": ["overall success rate and what drives it", "what specifically breaks consistency", "the story you tell yourself about the habit", "what succeeding vs failing feels like emotionally", "what would change in your life if this habit was solved"],
            "avoidance_detection":  ["what exactly is being avoided", "the real cost of continued avoidance", "what you fear finding if you look", "the smallest possible step toward it", "when this avoidance pattern started"],
            "forward_planning":     ["one concrete action for tomorrow", "the specific obstacle you predict", "who in your life could support this", "what has actually worked before in similar situations", "what success would look and feel like"],
        }
        angle_idx = session_count % 5
        cat_angle_block = "\n".join(
            f"  {cat}: this session focus on → '{angles[angle_idx]}'"
        ...
        try:
            ...
            raw = loop.run_until_complete(
                app_ctx._call_llm_with_context(
                    ...
                    context=[],
                    intrinsic_only=True,
                    system_override="You are Arwanos. Output ONLY a valid JSON array of 5 questions. No prose, no markdown, no explanation."
                )
            )
            loop.close()
        ...
        if not questions:
            questions = [
                {"id": "q1", "text": "How are you feeling emotionally right now, and what's driving that?", "category": "emotional_awareness"},
                {"id": "q2", "text": "What pattern in your behavior have you noticed most this week?", "category": "behavior_patterns"},
                {"id": "q3", "text": "How consistent have you been with your habits and commitments?", "category": "habit_accountability"},
                {"id": "q4", "text": "Is there something you've been avoiding thinking about or dealing with?", "category": "avoidance_detection"},
                {"id": "q5", "text": "What is one specific action you will take tomorrow to move forward?", "category": "forward_planning"},
            ]

        # ── PROGRAMMATIC DE-DUPLICATION ───────────────────────────────────────
        _DUP_STOP = {
            'what','when','where','which','have','been','your','with','from','they',
            'will','more','most','some','make','does','like','just','into','over',
            'than','them','then','were','also','both','each','such','take','time',
            ...
            'during','while','after','before','since','until','through','between',
            'describe','consider','reflect','explore','discuss','share','tell',
            'could','would','should','their','there','here','often','specific',
            'situation','particular','feelings','thoughts','please','often',
        }

        def _char_ngrams(text: str, n: int = 4) -> set:
            t = _re2.sub(r'\s+', ' ', text.lower())
        ...
        def _key_nouns(text: str) -> set:
            # Meaningful content words (6+ chars, not stopwords)
        ...
        if duplicates_idx:
            ...
            try:
                ...
                raw2 = loop2.run_until_complete(
                    app_ctx._call_llm_with_context(
                        query=regen_prompt,
                        conversation_history=[],
                        context=[],
                        intrinsic_only=True,
                        system_override="You are Arwanos. Output only a valid JSON array. No prose."
                    )
                )
                loop2.close()
                m2 = _re2.search(r'\[.*\]', raw2, _re2.DOTALL)
                if m2:
        ...
        for i, q in enumerate(questions, 1):
            q["id"] = f"q{i}"
            q.pop("_regenerated", None)

        session_id = f"sess_{today.strftime('%Y%m%d')}_{uuid.uuid4().hex[:6]}"
        session = {
        ...
        return jsonify({
            ...
            "session": session,
            "tuning_profile": profile,
            "dedup_count": len(duplicates_idx),   # how many questions were regenerated
        })

    @app.post("/api/monitor/session/answer")
    def api_monitor_session_answer():
        import datetime as _dt
        data = request.get_json(silent=True) or {}
        ...
        session["answers"] = [a for a in session.get("answers", []) if a.get("question_id") != question_id]
        ...
        if q:
            cat = q.get("category", "general")
            engaged = session.setdefault("topics_engaged", [])
            avoided = session.setdefault("topics_skipped", [])
            if skipped:
                if cat not in avoided:
                    avoided.append(cat)
                if cat in engaged:
                    engaged.remove(cat)
            else:
    ...
    @app.post("/api/monitor/session/complete")
    def api_monitor_session_complete():
        import asyncio, datetime as _dt
        data = request.get_json(silent=True) or {}
        session_id = data.get("session_id")
        mood_end = float(data.get("mood_end", 5.0))
        ...
        session["mood_end"] = mood_end
        session["completed_at"] = _dt.datetime.now().isoformat()
        session["status"] = "completed"

        # Build Q&A text from this session
        qa_text = ""
        answers_text_only = []
        ...
        # ── PIPELINE PHASE 1: JOURNAL CROSS-REFERENCE ───────────────────────
        # Compare session answers against ALL journal entries to find
        # confirmed patterns, contradictions, new revelations, and progression.
        all_entries = _read_entries()
        all_entries_sorted = sorted(all_entries, key=lambda e: e.get("date", ""), reverse=True)

        journal_timeline = "\n".join([
            f"[{e.get('date')}] Mood:{e.get('mood','?')} | {e.get('title','')} | {(e.get('details','') or '')[:250]}"
            for e in all_entries_sorted[:20]  # most recent 20 in full
        ])
        older_titles = "\n".join([
            f"[{e.get('date')}] Mood:{e.get('mood','?')} | {e.get('title','')}"
            for e in all_entries_sorted[20:]
        ])

        inference_prompt = (
            "You are Arwanos performing a cross-reference analysis.\n"
            "Compare today's session answers against the complete journal history.\n"
            "Find what the answers CONFIRM, CONTRADICT, or REVEAL that wasn't visible in the journals alone.\n\n"
        ...
        try:
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            raw_inf = loop.run_until_complete(
                app_ctx._call_llm_with_context(
            ...
            loop.close()
            import re as _re3
            m = _re3.search(r'\{.*\}', raw_inf, _re3.DOTALL)
            if m:
        ...
        enriched_search_text = " ".join([
            " ".join(journal_inferences.get("confirmed_patterns", [])),
            " ".join(journal_inferences.get("new_revelations", [])),
            " ".join(journal_inferences.get("contradictions", [])),
            journal_inferences.get("key_inference", ""),
            " ".join(answers_text_only[:2]),   # anchored by actual answers too
        ])
        psych_examples = _psych_keyword_search(enriched_search_text, top_k=4)

        # ── PIPELINE PHASE 3: FINAL INSIGHTS ────────────────────────────────
        profile_block = ""
        if tuning_profile:
            profile_block = (
        ...
        inference_block = (
            f"JOURNAL CROSS-REFERENCE FINDINGS:\n"
            f"  Confirmed patterns: {', '.join(journal_inferences.get('confirmed_patterns', []))}\n"
        ...
        try:
            ...
            insights = loop.run_until_complete(
                app_ctx._call_llm_with_context(
                    ...
                    context=[],
                    intrinsic_only=True,
                    system_override="You are Arwanos, a psychoanalytical monitor. Write session insights as plain paragraphs."
                )
            )
            loop.close()
        except Exception as e:
            insights = f"Session completed. AI insights unavailable: {e}"
        ...
        stats["sessions_completed"] = stats.get("sessions_completed", 0) + 1
        completed = stats["sessions_completed"]
        prev_avg = stats.get("avg_mood_improvement", 0.0)
        stats["avg_mood_improvement"] = round((prev_avg * (completed - 1) + mood_delta) / completed, 2)
        progress["last_updated"] = _dt.date.today().isoformat()

        for cat in session.get("topics_engaged", []):
            t = progress["topics"].setdefault(cat, {
                "label": cat.replace("_", " ").title(),
                "times_engaged": 0, "times_skipped": 0,
                "last_engaged": None, "progress_score": 0.0,
            })
            t["times_engaged"] = t.get("times_engaged", 0) + 1
            t["last_engaged"] = session.get("date")

        for cat in session.get("topics_skipped", []):
            t = progress["topics"].setdefault(cat, {
                "label": cat.replace("_", " ").title(),
                "times_engaged": 0, "times_skipped": 0,
                "last_engaged": None, "progress_score": 0.0,
            })
            t["times_skipped"] = t.get("times_skipped", 0) + 1

        for t in progress["topics"].values():
            eng = t.get("times_engaged", 0)
            skip = t.get("times_skipped", 0)
            total = eng + skip
            t["progress_score"] = round(eng / total, 2) if total else 0.0

        _write_monitor_progress(progress)
        return jsonify({"ok": True, "insights": insights, "journal_inferences": journal_inferences, "session": session})

    @app.get("/api/monitor/progress")
    def api_monitor_progress():
        return jsonify({"ok": True, "progress": _read_monitor_progress()})
...
class ArwanosApp(ctk.CTk, CommandRouterMixin):
    ...
    def __init__(self):
        ...
        # Always initialize self.paths — _switch_demo_mode sets it for both modes
        self._switch_demo_mode(self._demo_mode)
        ...
        try:
            ...
            # Zoom state — auto-detect monitor count for default font size
            self._current_zoom_delta = _detect_initial_zoom_delta()
        ...
        self._vo_proc    = None
        self._vo_anim_id = None
        self._vo_stop_btn = ctk.CTkButton(
            self,
            text="🔊 Speaking…  ✕ Stop",
        ...
        # Not placed yet — _speak_response() will call place() when TTS starts
    ...
    def _insert_user_line(self, text: str):
        ...
        if has_arabic(s):
            ...
            try:
                raw.insert("end", f"{_get_username()}:\n", ("user",))
                raw.insert("end", shaped + "\n\n", ("user", "rtl"))
                self.chat_history.see("end")
            except Exception:
                self.chat_history.insert("end", f"{_get_username()}:\n{shaped}\n\n", ("user", "rtl"))
                self.chat_history.see("end")
            return

        self.chat_history.insert("end", f"{_get_username()}: {shaped}\n\n", ("user",))
        self.chat_history.see("end")
    ...
    def _refresh_zoom_extras(self, base_pt: int) -> None:
        ...
        try:
            raw = getattr(self.chat_history, "_textbox", self.chat_history)
            # Pick the first monospace font that is actually installed
            import tkinter.font as _tkf
            _installed = set(_tkf.families())
            _mono_name = next(
                (f for f in ("Cascadia Code", "DejaVu Sans Mono",
                             "Liberation Mono", "Courier New", "Courier",
                             "Lucida Console", "Consolas")
                 if f in _installed),
                "TkFixedFont"
            )
            mono = (_mono_name, max(10, base_pt - 1))
    ...
    def _apply_highlights_from_payload(
        ...
        for h in highlights:
            ...
            # Relative position for disambiguation when snippet appears multiple times
            rel = (start / len(saved)) if (len(saved) > 0 and start >= 0) else 0.5
            expected_start = int(rel * text_len)

            def _best_span(candidates):
                """Pick candidate (s, e) closest to expected_start, skipping overlaps."""
                best, best_dist = None, float("inf")
                for s, e in candidates:
                    if any((s < b and e > a) for a, b in used_spans):
                        continue
                    d = abs(s - expected_start)
                    if d < best_dist:
                        best, best_dist = (s, e), d
                return best

            # 1) Exact offsets — only trust them when transcript lengths match exactly.
            if 0 <= start < end and len(saved) == text_len:
                if text[start:end] == snippet:
                    _add_span(start, end, color)
                    continue

            # 2) Exact string search — most reliable; avoids over-matching across paragraphs.
            exact_hits = []
            pos = 0
            while True:
                ...
                if idx == -1:
                    break
                exact_hits.append((idx, idx + len(snippet)))
                pos = idx + 1

            if exact_hits:
                span = _best_span(exact_hits)
                if span:
                    _add_span(span[0], span[1], color)
                continue

            # 3) Flexible-whitespace regex — fallback for minor whitespace differences.
            #    Only used if exact search found nothing. Anchored to first/last word to
            #    prevent the pattern from matching across unrelated paragraphs.
            words = [w for w in _re.split(r"\s+", snippet) if w]
            if not words:
                continue
            # Limit to first+last 4 words so the pattern stays narrow
            anchor_words = words[:4] + (words[-4:] if len(words) > 4 else [])
            escaped = [_re.escape(w) for w in anchor_words]
            pattern = r"\s+".join(escaped)
            try:
                flex_hits = [(m.start(), m.end()) for m in _re.finditer(pattern, text)]
            except Exception:
                flex_hits = []

            if flex_hits:
                span = _best_span(flex_hits)
                if span:
                    _add_span(span[0], span[1], color)
                continue

            # 4) Last resort: apply saved offsets if they're within bounds.
    ...
    def _show_comments_list(self):
        ...
        ctk.CTkButton(
            ...
            text_color="#c4b5fd",
            border_color="#4c1d95", border_width=1, corner_radius=7,
    ...
    def _search_in_chat(self, query: str):
        ...
        Uses Tk's native .search() for pixel-accurate index positions —
        avoids the offset drift caused by embedded windows (copy buttons etc.)
        when using Python re + char-offset conversion.
        """
        self._clear_search_highlights()
        self._search_matches = []

        if not getattr(self, "chat_history", None) or not query:
            return

        # Always operate on the inner tk.Text widget for accuracy
        raw = getattr(self.chat_history, "_textbox", self.chat_history)

        try:
            raw.tag_config("search_highlight",
                           background="#e63946", foreground="#ffffff")
        except Exception:
            pass
        ...
        try:
            ...
            start_pos = "1.0"
            while True:
                pos = raw.search(query, start_pos, stopindex="end", nocase=True)
                if not pos:
                    break
                end_pos = f"{pos}+{qlen}c"
                try:
                    raw.tag_add("search_highlight", pos, end_pos)
                except Exception:
                    pass
                # Normalise to canonical row.col form before storing
                self._search_matches.append(
                    (raw.index(pos), raw.index(end_pos))
                )
                start_pos = end_pos   # advance past this match
        except Exception:
            pass
    ...
    def _navigate_to_current_match(self):
        ...
        try:
            raw.tag_remove("current_match", "1.0", "end")
        except Exception:
            pass

        try:
            # Active match: bright orange so it stands out from the red matches
            raw.tag_config("current_match", background="#ff9f1c", foreground="#000000")
            raw.tag_add("current_match", start_idx, end_idx)
            raw.tag_raise("current_match")
        except Exception:
            pass
    ...
    def _normalize_history_for_export(self) -> list[dict]:
        ...
        """
        hist = getattr(self, "conversation_history", []) or []
        # Strip housekeeping messages that are meaningless in a saved file
        _SKIP_PREFIXES = ("Session saved to ", "Saved ", "Reloading vault")
        hist = [
            h for h in hist
            if not any(
                (h.get("content","") if isinstance(h,dict) else str(h)).lstrip()
                .startswith(p) for p in _SKIP_PREFIXES
            )
        ]
    ...
    def _show_large_file_picker(self, title: str, initialdir: str,
                                  extensions: tuple = (".json", ".txt")) -> str | None:
        """Large custom file picker (760×520) — readable on laptop screens."""
        from pathlib import Path as _P
        chosen   = [None]
        cwd      = [_P(initialdir) if _P(initialdir).is_dir() else _P.home()]
        ...
        _font_lbl  = ("DejaVu Sans", 14)
        _font_file = ("DejaVu Sans", 15)

        # Header: path label + Up button
        hdr = ctk.CTkFrame(dlg, fg_color="transparent")
        hdr.pack(fill="x", padx=12, pady=(10, 4))
        dir_lbl = ctk.CTkLabel(hdr, text="", anchor="w", font=_font_lbl,
                               wraplength=640, justify="left")
        dir_lbl.pack(side="left", fill="x", expand=True)
        btn_up = ctk.CTkButton(hdr, text="↑ Up", width=80,
                               command=lambda: _nav(cwd[0].parent))
        btn_up.pack(side="right", padx=(6, 0))

        # Scrollable file list
        list_frame = ctk.CTkScrollableFrame(dlg, fg_color="#111122")
        list_frame.pack(fill="both", expand=True, padx=12, pady=4)

        # Manual path entry
        entry_var = ctk.StringVar()
        entry = ctk.CTkEntry(dlg, textvariable=entry_var, font=_font_lbl,
                             placeholder_text="File name…", height=38)
        entry.pack(fill="x", padx=12, pady=(4, 2))

        def _nav(new_dir: "_P") -> None:
            if new_dir.is_dir():
                cwd[0] = new_dir
                _refresh()
        ...
        def _confirm() -> None:
            ...
            if val and _P(val).is_file():
                chosen[0] = val
            if chosen[0]:
                dlg.destroy()

        def _refresh() -> None:
            dir_lbl.configure(text=str(cwd[0]))
            ...
            try:
                ...
                return
            for p in items:
                if p.is_file() and p.suffix.lower() not in extensions and extensions:
                    continue
                icon = "📁  " if p.is_dir() else "📄  "
                btn = ctk.CTkButton(
                    list_frame, text=icon + p.name,
                    anchor="w", fg_color="transparent",
                    hover_color="#1e2a4a",
                    font=_font_file,
                    height=36,
                    command=lambda _p=p: _pick(_p)
                )
        ...
        ctk.CTkButton(btn_row, text="Open", width=130,
                      fg_color="#0b5394", hover_color="#0f6ac2",
    ...
    def _prompt_import_session(self):
        ...
        try:
            ...
            pass
        path = self._show_large_file_picker(
    ...
    def _render_markdown_to_chat(self, text: str, speaker: str, base_tag: str):
        ...
        def _insert_table(table_lines):
            ...
            Long cells are word-wrapped so the table never exceeds readable width.
            Uses plain ASCII borders (+-) so any font aligns correctly.
            """
            import textwrap as _tw, re as _re_tbl
            try:
                def _strip_md(t: str) -> str:
                    """Strip **bold**, *italic*, `code` markers from a cell."""
                    t = _re_tbl.sub(r"\*\*(.+?)\*\*", r"\1", t)
                    t = _re_tbl.sub(r"\*(.+?)\*",     r"\1", t)
                    t = _re_tbl.sub(r"`(.+?)`",        r"\1", t)
                    return t.strip()
                ...
                for i, ln in enumerate(table_lines):
                    if _is_separator_line(ln):
                        ...
                        continue
                    cells = [_strip_md(c) for c in ln.strip().strip("|").split("|")]
                    rows.append(cells)
                ...
                # Column width caps — first col (label) narrower, rest wrap at 50
                _MAX = [22] + [50] * (ncols - 1)
                natural = [max(len(r[c]) for r in rows) for c in range(ncols)]
                widths  = [max(min(natural[c], _MAX[c]), 4) for c in range(ncols)]

                def _wrap_cell(text: str, w: int) -> list:
                    if len(text) <= w:
                        return [text]
                    return _tw.wrap(text, width=w, break_long_words=True) or [""]

                def _render_row(row: list) -> list:
                    wrapped  = [_wrap_cell(row[c], widths[c]) for c in range(ncols)]
                    n_lines  = max(len(wc) for wc in wrapped)
                    out = []
                    for li in range(n_lines):
                        parts = []
                        for c in range(ncols):
                            cell_line = wrapped[c][li] if li < len(wrapped[c]) else ""
                            parts.append(cell_line.ljust(widths[c]))
                        out.append(" | " + " | ".join(parts) + " |\n")
                    return out

                # ASCII borders — work with any font
                def _top():
                    segs = ["-" * (widths[c] + 2) for c in range(ncols)]
                    return " +" + "+".join(segs) + "+\n"
    ...
    def _classify_intent(self, q: str) -> str:
        ...
        if not t:
            return "open"
        # code requests — must short-circuit BEFORE any other check so that
        # queries like "give me html code for: ... <tr> defines a row ..."
        # are never misclassified as "define" due to "defines" substring.
        if any(
            p in t
            for p in [
                "give me code",
        ...
        # NOTE: use whole-word check for "define" to avoid matching "defines"
        # inside quoted text the user is asking about.
        _define_substr = [
            "what is",
            "what's",
            "what does",
        ...
        if any(p in t for p in _define_substr):
            return "define"
        # "define" as a standalone command word (not "defines", "defined", etc.)
        if _re_ic.search(r'\bdefine\b', t):
            return "define"
    ...
    def _intent_system_prompt(self, intent: str) -> str:
        ...
        _BASE = (
            ...
            "IMPORTANT: /rag, /lo, /hunt, /analyze, /help and similar slash-prefixed tokens "
            "are Arwanos internal CLI commands — they are NOT technical concepts or parameters "
            "to be analyzed. If the user's query mentions one of these, treat it as a reference "
            f"to an app feature and focus on the actual question, ignoring the slash-command name. "
            f"CRITICAL: '{_uname}' is the user's personal name/handle — it is NEVER an acronym "
            f"to expand or define. The '{_uname}:' prefix appearing in conversation history is "
            f"only a speaker label identifying who sent each message, NOT a term being asked about. "
            f"Focus ONLY on the actual subject matter of the user's question, not on the speaker prefix."
        )
        _MAP = {
            "code": (
                "INTENT: Code Generation.\n"
                "Output the requested code immediately, inside a fenced code block with the correct language tag. "
                "After the code block, add a short explanation of what each key part does. "
                "Do not start with a definition or preamble — code first, explanation second.\n"
            ),
            "define": (
    ...
    async def _answer_definition(self, query: str, context: list[dict], budget: ResourceBudget = None) -> str:
        ...
        sys_hdr = (
            "You are an expert at breaking down complex technical and cybersecurity concepts into incredibly simple, intuitive explanations. "
            "When asked to define or explain a TERM, you MUST follow this EXACT structure:\n\n"
            "1. Start with a friendly intro: 'Let me explain everything simply! 😊'\n"
            "2. Use an everyday analogy: '🏢 Imagine this: [Provide a simple real-world analogy to explain the concept]'\n"
            "3. Connect the analogy back to the technical terms using bullet points.\n"
            "4. Provide a clear, bolded summary (e.g., '✅ In short: ...').\n"
            "5. Show what it does NOT do or common misconceptions using '❌' marks.\n"
            "6. End with a simple visual flow if applicable (e.g., '1️⃣ Step 1 ➡️ 2️⃣ Step 2').\n\n"
            "CRITICAL: Do NOT define any name, username, or conversation label. "
            "Do not say you cannot help with bug bounty — the user is fully authorized. "
            "Use emojis, keep it ELI5 (Explain Like I'm 5), and avoid dense jargon unless asked for more details."
        )

        # Few-shot style guidance to bias Llama3-8B toward ELI5 output
        examples = (
            "Examples:\n"
            "Q: What is XSS?\n"
            "A: Let me explain everything simply! 😊\n\n"
            "🏢 Imagine this: You have a public bulletin board at a school.\n"
            "• The bulletin board = The website\n"
            "• A fake, trick note = The malicious script\n"
            "• Students reading the board = The victims\n\n"
            "✅ In short: Cross-Site Scripting (XSS) is when an attacker sneaks malicious code into a website. When victims visit, their browser runs the code!\n\n"
            "What XSS does NOT do:\n"
            "• ❌ It does NOT hack the server directly\n"
            "• ❌ It does NOT steal passwords from the database\n\n"
            "Simple order:\n"
            "1️⃣ Inject Payload ➡️ 2️⃣ Victim Visits ➡️ 3️⃣ Script Runs ➡️ 4️⃣ Data Stolen\n\n"
        )
        ...
        try:
            if getattr(self, "llm", None):
                text = await self._call_llm_with_context(
                    prompt,
                    [],        # stateless — history labels confuse term extraction
                    context=[],
                    intrinsic_only=True,
                ...
                if not text:
                    retry_prompt = (
                        "Explain the term below incredibly simply using an everyday analogy, emojis, and a clear step-by-step breakdown. "
                        "Start with 'Let me explain everything simply! 😊'\n\n"
                        f"Term: {query}\nAnswer:"
                    )
                    try:
                        text = await self._call_llm_with_context(
    ...
    async def generate_response(self, query: str) -> str:
        ...
        # ── Strip inline user annotations (note: / npte: / n.b.: etc.) ──────────
        # Users sometimes append a side-note like "npte: /rag is just a command".
        # These are meta-comments for context, not part of the actual question.
        import re as _re_q
        q = _re_q.sub(
            r"\s{1,4}n[a-z]*e?\s*:.*$",   # "note:", "npte:", "nb:", "npt:", etc.
            "",
            q,
            flags=_re_q.IGNORECASE,
        ).strip()
        if not q:
            return "Empty message."
        ...
        if intent_str == "code":  intent["requires_reasoning"] = True
    ...
    def _send_message_text(self, msg: str, *, _skip_auto_forward: bool = False) -> None:
        ...
        if hasattr(self, "_insert_user_line"):
            try:
                ...
                try:
                    self.chat_history.insert("end", f"{_get_username()}: {msg}\n\n", ("user",))
                    self.chat_history.see("end")
                except Exception:
                    pass
        ...
        *,
        ...
        def _hist_line(turn) -> str:
            ...
            if isinstance(turn, dict):
                ...
                if not content:
                    return ""
                prefix = "Arwanos" if role.startswith("assist") else _get_username()
            ...
            if isinstance(turn, str):
                s = turn.strip()
                return f"{_get_username()}: {s}" if s else ""
    ...
    def _reply_assistant(self, text: str, *, _skip_bridge: bool = False):
        ...
        # /vo mode: auto-speak this response (one-shot, then clear flag)
        if getattr(self, "_vo_pending", False):
            _is_ui_noise = any(
                text.startswith(p) for p in getattr(self, "_UI_NOISE_PATTERNS", ())
            )
            if not _is_ui_noise and len(text.strip()) > 10:
                self._vo_pending = False
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # /ques — Study Quiz Generator
    # ═══════════════════════════════════════════════════════════════════════════

    async def _cmd_ques_async(self, arg: str) -> None:
        """
        /ques — scans imported session and prints a ready-to-use study command list.
        """
        import re as _re
        import logging as _log
        from collections import Counter
        ...
        if not idx or not history:
            self._reply_assistant(
                "No session loaded. Import a session first, then run /ques."
            )
            return

        self.update_status("Scanning session topics...")

        # --- extract user questions from session ---
        user_turns = []
        for turn in history:
            if isinstance(turn, dict) and turn.get("role") == "user":
                content = (turn.get("content") or "").strip()
                # skip very short or slash-command turns
                if len(content) > 15 and not content.startswith("/"):
                    user_turns.append(content)

        if not user_turns:
            self._reply_assistant("Could not extract topics from session.")
            self.update_status("Ready")
            return

        # --- ask LLM to extract clean topic list from user turns ---
        llm = getattr(self, "llm", None)
        if not llm:
            self._reply_assistant("LLM unavailable.")
            return

        sample = "\n".join(f"- {t[:120]}" for t in user_turns[:40])

        system = (
            "You are a study assistant. Given a list of study questions/topics from a user's session, "
            "extract the TOP 10 most important and distinct topics as short phrases (2-5 words each). "
            "Output ONLY a numbered list like:\n"
            "1. topic name\n"
            "2. topic name\n"
            "...and nothing else. No explanation, no headers."
        )
        prompt = f"{system}\n\nSession questions:\n{sample}\n\nTop 10 topics:"

        try:
            kwargs = {"options": {"num_predict": 200}}
            if hasattr(llm, "ainvoke"):
                r = await llm.ainvoke(prompt, **kwargs)
                raw = (getattr(r, "content", None) or str(r or "")).strip()
            else:
            ...
            _log.error(f"/ques topic extraction failed: {e}")
            raw = ""
        ...
        if not topics:
            self._reply_assistant("Could not parse topics from session.")
            self.update_status("Ready")
            return

        # --- build command menu ---
        lines = ["**Study Commands — based on your session**\n"]
        lines.append("── Review (from your notes) ──")
        for t in topics:
            lines.append(f"  /rag {t}")

        lines.append("\n── Quiz yourself ──")
        for t in topics[:5]:
            lines.append(f"  /ques quiz {t}")

        lines.append("\n── Go deeper (live web) ──")
        for t in topics[:5]:
            lines.append(f"  /deep {t} exam questions")

        lines.append("\n── Which command to use? ──")
        lines.append("  /rag <topic>        → answer from YOUR notes")
        lines.append("  /ques quiz <topic>  → get an MCQ question on that topic")
        lines.append("  /deep <topic>       → live web search for gaps in your notes")

        self._reply_assistant("\n".join(lines))
        self.update_status("Ready")

    async def _cmd_ques_quiz_async(self, topic: str) -> None:
        """Generate a single MCQ question from session notes on a topic."""
        import logging as _log

        idx = getattr(self, "_session_idx", None)
        history = getattr(self, "conversation_history", [])
        if not idx or not history:
            self._reply_assistant("No session loaded.")
            return

        self.update_status(f"Generating question on: {topic}...")

        import re as _re
        ql = topic.lower()
        terms = [t for t in _re.split(r"[^\w]+", ql) if t and len(t) > 2]
        seen: set = set()
        for term in terms:
            for ti in idx.get(term, []):
                seen.add(ti)
        ...
        if not top:
            self._reply_assistant(f"No content found for **{topic}**.")
            self.update_status("Ready")
            return
        ...
        system = (
            "You are Arwanos, a study coach. Using ONLY the session notes, "
        ...
        try:
            kwargs = {"options": {"num_predict": 350}}
            if hasattr(llm, "ainvoke"):
                r = await llm.ainvoke(prompt, **kwargs)
                text = (getattr(r, "content", None) or str(r or "")).strip()
            else:
            ...
            _log.error(f"/ques quiz failed: {e}")
            text = ""

        if text:
            self._reply_assistant(f"**Quiz — {topic}**\n\n{text}")
        else:
            self._reply_assistant("Could not generate a question. Try again.")
        self.update_status("Ready")
    ...
    def _cmd_clear(self, _):
        ...
        if not messagebox.askyesno("Confirm Clear", "Are you sure you want to clear the chat? This cannot be undone."):
            return
