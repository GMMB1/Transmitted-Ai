...
const Storage = {
    ...
    async init() {
        try {
            ...
            // Habits always come from Flask API (data/habits.json or data_test/habits.json).
            // 204 = file absent → empty list for this folder.
            const apiHabits = await this.fetchHabitsFromAPI();
            this.habits = apiHabits !== null ? apiHabits : [];
            if (apiHabits !== null) console.log('Habits loaded from Flask API (habits.json)');
        } catch (error) {
    ...
    async save() {
        try {
            // Habits are intentionally excluded — they live in data/habits.json
            // or data_test/habits.json (per-folder, via Flask API).
            // Including them here caused demo habits to contaminate personal localStorage.
            const data = {
    ...
    async function _checkMode() {
        try {
            ...
            if (version !== _lastVersion) {
                ...
                //       habits are restored when switching back from demo mode.
