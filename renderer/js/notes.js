...
const Notes = {
    ...
    hideView() {
        ...
        const overlay = document.getElementById('popup-overlay');
        if (overlay) overlay.classList.remove('solid');
    },
    ...
    async showList() {
        ...
        notes.sort((a, b) => {
            ...
            // Struck notes always go to the bottom
        ...
        notes.forEach(note => {
            ...
            card.className = 'report-card note-card' + (isStruck ? ' note-struck' : '') + (isPinned ? ' note-pinned' : '');
            ...
            card.innerHTML = `
                ...
                <div class="report-actions" style="margin-top: 15px; display: flex; gap: 5px; flex-wrap: wrap;">
                    ...
                    <button class="secondary-btn btn-pin" title="${isPinned ? 'Unpin note' : 'Pin to top'}"
                        style="padding: 5px 10px; font-size:0.8rem; ${isPinned ? 'background:rgba(255,170,0,0.18); color:#ffaa00;' : ''}">
                        ${isPinned ? '📌 Unpin' : '📌 Pin'}
                    </button>
            ...
            card.querySelector('.btn-view').addEventListener('click', (e) => {
                e.preventDefault();
                // Hide list panel only — keep overlay alive so the journal page never shows through
                if (this.listModal) {
                    this.listModal.classList.remove('active');
                    this.listModal.style.display = '';
                }
                this.viewNote(note);
            });
            ...
            card.querySelector('.btn-edit').addEventListener('click', (e) => {
                e.preventDefault();
                // Same: keep overlay alive during the transition
                if (this.listModal) {
                    this.listModal.classList.remove('active');
                    this.listModal.style.display = '';
                }
            ...
            card.querySelector('.btn-pin').addEventListener('click', async (e) => {
                e.preventDefault();
                await this.togglePinned(note.id);
            });
    ...
    async togglePinned(id) {
        try {
            const res = await fetch(`/api/notes/${id}/pinned`, { method: 'PATCH' });
            if (res.ok) {
                const data = await res.json();
                const msg = data.pinned ? 'Note pinned to top.' : 'Note unpinned.';
