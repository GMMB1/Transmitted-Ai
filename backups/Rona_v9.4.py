                ...
                if isinstance(turns, list):
                    for t in turns:
                        ...
                        if role in ("user", "gmm") and txt and not q:
                            q = txt
                        if role in ("rona", "assistant") and txt and not a:
                            a = txt
...
class RonaAppEnhanced(ctk.CTk, CommandRouterMixin):
    ...
    def _on_dragon_btn_click(self):
        ...
        self._play_dragon_sound("assets/532155__soundmast123__mighty-dragon-roaring.wav")
    ...
    def _start_dragon_phase2(self):
        ...
        self._play_dragon_sound("assets/427248__get_accel__49-dragon-roar.wav")
    ...
    def _play_dragon_sound(self, path: str) -> None:
        """Play a sound file non-blocking. Uses pygame if available, else aplay/paplay."""
        import os, threading

        if not os.path.exists(path):
            print(f"[dragon sound] file not found: {path}")
            return
        ...
        def _play():
            ...
            try:
                ...
                pass
            # Fallback: subprocess (Linux aplay / paplay)
            import subprocess
            for player in ("paplay", "aplay", "afplay"):
                try:
                    ...
                    continue
            print("[dragon sound] no audio player found (tried paplay/aplay/afplay)")
    ...
    def _show_dragon_splash(self, path: str, duration_ms: int = 2000, on_done=None):
        ...
        try:
            ...
            if not os.path.exists(path):
                print(f"[dragon] file not found: {path}")
                self._dragon_active = False
                if callable(on_done):
                    self.after(0, on_done)
                return

            # Create borderless always-on-top window, centered on the Rona window
    ...
    def _show_four_dragons(self, duration_ms: int = 5000):
        ...
        dragons = [
            ...
            ("assets/dragon.gif",          0,    0, (320, 240)),
            # ── SURROUNDING 3 ───────────────────────────────────────────
            ("assets/dragon-top.gif",    -210, -190, (200, 150)),  # top-left
            ("assets/dragon-right.gif",   210, -190, (200, 150)),  # top-right
            ("assets/dragon_bottom.gif",    0,  210, (200, 150)),  # bottom-center
        ]
        ...
        for path, ox, oy, (w, h) in dragons:
            if not os.path.exists(path):
                ...
                continue
            try:
                frames, delays = self._load_gif_frames_with_durations(path, size=(w, h))
                if not frames:
                    continue
