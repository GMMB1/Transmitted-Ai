...
class _BugBountyShim:
    ...
    def run_lovely_mode(self, folder_path: str) -> Dict[str, Any]:
        return {
            "success": False,
            "error": "Lovely mode disabled.",
            "stdout": "",
            "stderr": "",
        }
...
def _help_text() -> str:
    return (
        ...
        "- `/bash <cmd>` — confirm then run shell command\n"
...
async def _call_llm_with_context(
    ...
    *,
    ...
    def _sync_call() -> str:
        try:
            if hasattr(llm, "invoke"):  # modern sync
                r = llm.invoke(prompt)
                return (getattr(r, "content", None) or str(r or "")).strip()
            if hasattr(llm, "generate"):  # some wrappers
                r = llm.generate([prompt])
                if hasattr(r, "generations") and r.generations and r.generations[0]:
                    return (getattr(r.generations[0][0], "text", "") or "").strip()
            if hasattr(llm, "predict"):  # very old LC
...
class CommandRouterMixin:
    ...
    def _init_commands(self):
        ...
        self._commands = {
            ...
            "/intrinsic": self._cmd_intrinsic,
            "/deep": self._cmd_deep,
            "/bash": self._cmd_bash,
            "/run": self._cmd_bash,  # alias
            "/poetic": self._cmd_poetic,
            "/lovely": self._cmd_lovely,
            "/lovelyq": self._cmd_lovelyq,  # async Q&A
            "/lovelypsycho": self._cmd_lovelypsycho,
            "/lovelydebug": self._cmd_lovelydebug,  # <— add this line
            "/hunt": self._cmd_hunt,
            "/memory": self._cmd_memory,
            "/webui": self._cmd_webui,  # start/stop flask
            "/predictive": self._cmd_predictive,  # open /prodectivity
        }
    ...
    def _cmd_deep(self, arg):
        if not arg:
            self._reply_assistant("Usage: /deep <query>")
            return
        import threading
    ...
    def _cmd_bash(self, arg):
        if not arg:
            self._reply_assistant("Usage: /bash <command>")
            return
        if hasattr(self, "confirm_and_run_shell"):
            self.confirm_and_run_shell(arg)
        else:
            self._reply_assistant("Shell runner not available in this build.")

    def _cmd_intrinsic(self, arg):
        if not arg:
            self._reply_assistant("Usage: /intrinsic <query>")
            return
        import threading

        threading.Thread(
            target=lambda: self._run_async(self._intrinsic_only_async(arg)), daemon=True
        ).start()
    ...
    def _cmd_lovelypsycho(self, arg: str):
        if not arg:
            self._reply_assistant("Usage: /lovelypsycho <long-text>")
            return
        import threading

        if hasattr(self, "_run_lovely_with_caching"):
            threading.Thread(
                target=self._run_lovely_with_caching, args=(arg,), daemon=True
            ).start()
        else:
            self._reply_assistant("Lovely psycho mode not available in this build.")

    def _cmd_memory(self, _arg=None):
        import threading

        if hasattr(self, "_show_memory_summary"):
            threading.Thread(target=self._show_memory_summary, daemon=True).start()
        else:
            self._reply_assistant("Memory summary not available.")

    def _cmd_lovelydebug(self, _arg: str):
        la = getattr(self, "lovely", None) or LovelyAnalyzer(self)
        info = la.debug_snapshot()
        if "error" in info:
            self._reply_assistant(f"Lovely debug error: {info['error']}")
            return
        msg = (
            "**Lovely Debug**\n"
            f"- Notes file: `{info['psycho_file']}`\n"
            f"- Notes count: {info['count']}\n"
            f"- Last note: {info['last_meta']}\n"
            f"- Last.details(head): {info['last_details_head']}\n"
            f"- Convo file: `{info['convo_file']}`\n"
            f"- Convo count: {info['convo_count']}"
        )
    ...
    def debug_snapshot(self) -> dict:
        try:
            ...
            return {
                "psycho_file": str(self._psycho_file),
                "count": len(entries),
                "last_meta": {k: last.get(k) for k in ("id", "title", "date", "mood")},
                "last_details_head": _head(last.get("details", "")),
                "convo_file": str(self._convo_file),
                "convo_count": len(self._read_convos()),
            }
        except Exception as e:
            return {"error": str(e)}

    def _cmd_hunt(self, arg):
        import threading

        if not arg:
            self._reply_assistant("Usage: /hunt <target>")
            return
        if hasattr(self, "_run_hunt_command"):
            threading.Thread(
                target=self._run_hunt_command, args=(arg,), daemon=True
            ).start()
        else:
            self._reply_assistant("Hunt integration not available.")
...
class LovelyAnalyzer:
    def __init__(self, app_ctx):
        ...
        self.paths = _find_repo_paths()
        self._convo_file: Path = (self.paths["data_dir"] / "lovely_conversations.json").resolve()
    ...
    def _append_convo(self, record: dict) -> None:
        ...
        try:
            ...
            if not isinstance(data, list):
                data = []
            data.append(record)
            self._convo_file.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")
        except Exception:
            pass
    ...
    async def analyze(self, user_question: str) -> str:
        ...
        try:
            text = await self.app_ctx._call_llm_with_context(
                ...
                context=[{"content": last_notes}] if last_notes else [],  # feed notes via context
    ...
    async def analyze_no(self, user_question: str) -> str:
        ...
        try:
            self._convo_file = (self.paths["data_dir"] / "lovely_conversations.json").resolve()
            self._convo_file.parent.mkdir(parents=True, exist_ok=True)
...
def _create_flask_app(app_ctx: "RonaAppEnhanced") -> Flask:
    ...
    # Single file, as you asked: ./data/psychoanalytical.json
    # We’ll reuse your LovelyAnalyzer helpers (read/write json) if you have them.
    # ---------------- JSON API (psychoanalytical.json) ----------------
    # Single file, as you asked: ./data/psychoanalytical.json
    # We’ll reuse your LovelyAnalyzer helpers (read/write json) if you have them.
    # ---------------- JSON API (psychoanalytical.json) ----------------
...
class RonaAppEnhanced(ctk.CTk, CommandRouterMixin):
    ...
    def __init__(self):
        ...
        try:
            ...
            try:
                ...
                pass
            # now apply the rest of the pretty styling exactly once
    ...
    def _ensure_llm(self):
        ...
        model_name = "llama3.1:8b-instruct"  # sensible default
        opts = {
            "temperature": 0.2,
            "num_ctx": 4096,
        ...
        # Prefer new config API
        if cfg and hasattr(cfg, "llm"):
            model_name = getattr(cfg.llm, "model", model_name)
            if hasattr(cfg.llm, "as_ollama_options"):
                opts = cfg.llm.as_ollama_options()

        # Otherwise, legacy AutoConfig if present
        elif hasattr(self, "legacy_cfg") and self.legacy_cfg:
            model_name = getattr(self.legacy_cfg, "model_name", model_name)
            if hasattr(self.legacy_cfg, "ollama_options"):
                opts = self.legacy_cfg.ollama_options()
    ...
    def send_message(self, event=None):
        ...
        if hasattr(self, "_append_conversation"):
            self._append_conversation("user", msg)
    ...
    def _push_hist(self, role: str, content: str) -> None:
        if not isinstance(getattr(self, "conversation_history", None), list):
            self.conversation_history = []
        self.conversation_history.append({"role": role, "content": (content or "").strip()})
    ...
    def _cmd_predictive(self, _):
        if not getattr(self, "webui", None):
            return "Web UI is not initialized."
        url = self.webui.open_predictive()
        return f"Opening Predictive UI… {url}"
    ...
    def _cmd_help(self, _):
        self._reply_assistant(
            ...
            "- `/help` — show this help\n"
            "- `/clear` — clear chat\n"
            "- `/intrinsic <q>` — model-only (no web)\n"
            "- `/deep <q>` — show top sources (no synthesis)\n"
            "- `/bash <cmd>` — run a shell command (if enabled)\n"
            "- `/poetic <topic>` — tiny poem\n"
            "- `/lovely <text>` — lovely flow (if enabled)\n"
            "- `/lovelydebug` — lovely debug (enabled)\n"
            "- `/hunt <target>` — bug-bounty flow (if enabled)\n"
            "- `/memory` — show memory (if enabled)\n"
    ...
    def _cmd_bash(self, arg: str):
        if not arg:
            return "Usage: `/bash <command>`"
        if hasattr(self, "confirm_and_run_shell"):
            self.confirm_and_run_shell(arg)
            return "Running… (see yellow terminal output below)"
        return "Shell runner is unavailable in this build."

    def _cmd_poetic(self, arg: str):
        topic = arg or "something gentle"
        text = (
            f"A hush of code, a spark of light,\n"
            f"your {topic} leans into the night.\n"
            f"We'll shape it clean, we’ll make it true —\n"
            f"one careful step, then something new."
        )
        self._reply_assistant(text)

    def _cmd_intrinsic(self, arg: str):
        if not arg:
            return "Usage: `/intrinsic <question>`"
        # model-only (no RAG) using existing call
        if hasattr(self, "_call_llm_with_context") and getattr(self, "llm", None):

            async def _run():
                out = await self._call_llm_with_context(
                    arg, self.conversation_history, context=[], intrinsic_only=True
                )
                self._reply_assistant(out or "No answer.")

            self._run_async(_run())
            return "Thinking (intrinsic)…"
        return "LLM backend not available."

    def _cmd_deep(self, arg: str):
        if not arg:
            return "Usage: `/deep <query>`"
        if hasattr(self, "_deep_search_and_reply"):
            threading.Thread(
                target=self._deep_search_and_reply, args=(arg,), daemon=True
            ).start()
            return "Searching deeply…"

        # fallback: use your generate_response but skip synthesis if you want
        async def _run():
            out = await self.generate_response(arg)
            self._reply_assistant(out)
        ...
        async def _run():
            ...
            la = (
                self._ensure_lovely()
                if hasattr(self, "_ensure_lovely")
                else getattr(self, "lovely", None) or LovelyAnalyzer(self)
            )
            try:
                ans = await la.analyze(arg)
            except Exception as e:
                ans = f"Lovely error: {e}"
            self._reply_assistant(ans)
            self.update_status("✅ Ready")
    ...
    def _cmd_hunt(self, arg: str):
        if not arg:
            return "Usage: `/hunt <target>`"
        if hasattr(self, "_run_hunt_command"):
            threading.Thread(
                target=self._run_hunt_command, args=(arg,), daemon=True
            ).start()
            return f"Hunting {arg}…"
        return "Hunt integration not available."
